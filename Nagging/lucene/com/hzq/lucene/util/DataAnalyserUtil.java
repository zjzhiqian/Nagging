...
public class DataAnalyserUtil {
	...
	public static void grabTianYaData(){
		grabTianYaData(TIANYA_URL);
	}
	/**
	 *  TODO GrabController的list添加数据,抓取完毕后更新FinishFlag
	 */
	public static void grabTianYaData(String url) {
		try {
			...
			try {
					try {
						HttpGet httpget = new HttpGet(TIANYA_URL);
	...
	private static void transFerContentToPost(String content) {
		...
		System.err.println(GrabController.getDataList());
		System.out.println(GrabController.getDataList().size());
